...
class AddRetailerSubscriptionScreen extends StatefulWidget {
  final String retailerId;
  final String retailerName;

  const AddRetailerSubscriptionScreen({
    super.key,
  ...
  @override
  State<AddRetailerSubscriptionScreen> createState() =>
      _AddRetailerSubscriptionScreenState();
}

class _AddRetailerSubscriptionScreenState
    extends State<AddRetailerSubscriptionScreen> {
...
Future<bool> isDuplicateTxn(String txn) async {
  ...
  final retailerSubs = await FirebaseFirestore.instance
  ...
  return retailerSubs.docs.isNotEmpty;
}
...
Future<String> uploadScreenshot() async {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final retailerRef = FirebaseFirestore.instance
          .collection('retailers')
          .doc(widget.retailerId);
      ...
      final safeDay = now.day > lastDayOfNextMonth
          ? lastDayOfNextMonth
          : now.day;
      ...
      // ✅ ADD SUBSCRIPTION
      await retailerRef.collection('subscriptions').add({
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'Retailer subscription added (Waiting admin approval)'),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Retailer Subscription'),
      ),
      body: Padding(
