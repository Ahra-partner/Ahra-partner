...
import 'admin/admin_payment_verification_screen.dart';

// 🔥 NEW IMPORT
import 'admin/admin_expiry_list_screen.dart';
...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  // 🔥 INITSTATE (FCM setup added)
  @override
  void initState() {
    super.initState();

    saveAdminToken(); // ✅ Step 3

    // ✅ Step 4: Topic subscribe
    FirebaseMessaging.instance.subscribeToTopic("admin");

    // ✅ Step 6: Foreground notification
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      print("🔔 Title: ${message.notification?.title}");
    });

    // ✅ Step 7: Notification click
    FirebaseMessaging.onMessageOpenedApp.listen((message) {
      Navigator.pushNamed(context, '/login'); // change if needed
    });
  }

  // 🔥 TOKEN SAVE FUNCTION (Step 3)
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const AdminExpiryListScreen(),
                ),
              );
            },
          ),
    ...
    builder: (context, snapshot) {
      ...
      if (!snapshot.hasData) {
        return ElevatedButton.icon(
          ...
          label: const Text('Withdraw Requests (0)'),
      ...
      return ElevatedButton.icon(
        icon: const Icon(Icons.account_balance_wallet),
        label: Text('Withdraw Requests ($count)'),
        style: _adminButtonStyle(const Color(0xFFDC2626)),
        onPressed: () {
          Navigator.push(
            context,
