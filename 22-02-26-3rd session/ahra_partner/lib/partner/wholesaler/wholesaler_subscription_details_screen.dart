...
class WholesalerSubscriptionDetailsScreen extends StatelessWidget {
  ...
  Future<void> _whatsAppWholesaler(String mobile) async {
    final uri = Uri.parse(
        'https://wa.me/91$mobile?text=Hello%20Wholesaler');
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri, mode: LaunchMode.externalApplication);
    }
  }
  ...
  Future<void> markAsPaid() async {
    ...
    await FirebaseFirestore.instance
      ...
      // 2️⃣ ✅ UPDATE WHOLESALER MAIN DOCUMENT
      tx.set(
        wholesalerRef,
        {
      ...
      tx.set(ledgerRef.doc(), {
        ...
        'direction': 'credit',
        'type':
            'wholesaler_subscription_commission',
        'description':
            'Wholesaler subscription commission',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          title: const Text(
              'Wholesaler Subscription Details')),
      body: FutureBuilder(
        future:
            Future.wait([wholesalerRef.get(), subRef.get()]),
